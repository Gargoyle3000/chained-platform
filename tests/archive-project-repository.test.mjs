...
test("Project input and private failures are sanitized before or after a request", async () => {
  ...
  const repository = createArchiveRepository(supabase, {});
  await assert.rejects(() => repository.createProject("   "), /SELECTOR IS CURRENTLY UNAVAILABLE/);
  await assert.rejects(() => repository.reorderProjectWorks(IDS.project, [IDS.workA, "not-a-work"]), /SELECTOR IS CURRENTLY UNAVAILABLE/);
  await assert.rejects(() => repository.addProjectWork(IDS.project, IDS.workA), /SELECTOR IS CURRENTLY UNAVAILABLE/);
