...
\- SELECTOR contains both automatic Works from directly managed Artist Profiles, independent of Work/Profile publication state, and published external Works explicitly saved or removed with the Work-level `[+]`; managed Works never render that control. Works remains canonical; Selector stores only private membership and organisation.

\- SELECTOR has derived ALL, PERSONAL and SAVED source scopes: PERSONAL is automatic Works from directly managed Artist Profiles, while SAVED is explicitly added external Works. These scopes are not user Tags; Tags remain secondary private filters, and Projects can mix both sources. Following never implicitly saves a Work.
