...
## Work publish-processing UX

- After a Work upload/edit, Publish can return HTTP 422 while required public derivatives are still processing, sometimes for up to roughly a minute. Repeated Publish attempts can fail until processing completes, after which Publish succeeds. The current UI may misleadingly show `WORK COULD NOT BE SAVED` even when the Work is already saved and only publication is blocked. Keep this open until the UI distinguishes save from Publish, communicates `WORK SAVED · PROCESSING IMAGES`, keeps Publish unavailable while known prerequisites are not ready, refreshes readiness automatically within a bounded interval, and surfaces a real failure/retry state for failed or stuck processing rather than waiting indefinitely.
