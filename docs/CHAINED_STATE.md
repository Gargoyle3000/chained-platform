...
- Authenticated workspace pages now provide permanent user-invoked contextual `[ ? ]` help as a quiet fixed bottom-left utility outside primary navigation. One responsive panel uses route-specific THIS PAGE / EDIT HERE / CONNECTED TO copy for Dashboard, Works, Presentations, Agenda, Profile, CV, Press and Archive, including their distinct editors; it is non-navigational and never opens automatically.
