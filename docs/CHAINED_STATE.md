...
- Compact public multi-image browsing is live on Discover, Following and Public Profile: those viewers use the same circular current-image state and compact green accessible `< current/total >` navigation (hidden for one-image Works), while preserving swipe/drag and keyboard browsing. Public Work Detail intentionally renders all ordered images vertically for close viewing/comparison and has no carousel. The control is UI-only and never enters exports.
