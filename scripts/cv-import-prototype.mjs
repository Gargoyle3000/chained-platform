...
export function validateCvImportResult(input) {
  ...
  if (input.source.documentKind !== "pdf" && input.source.documentKind !== "text") {
    throw new Error("source.documentKind must be pdf or text");
  }
  for (const key of ["pageCount", "extractedCharacterCount"]) {
    if (!Number.isInteger(input.source[key]) || input.source[key] < 0) {
  ...
  if (!Array.isArray(input.candidates) || input.candidates.length > MAX_CANDIDATES) {
    throw new Error(`candidates must be an array of at most ${MAX_CANDIDATES}`);
  }
  ...
  input.candidates.forEach((candidate, index) => {
    ...
    if (candidate.url !== null && candidate.url !== undefined && (typeof candidate.url !== "string" || !/^https?:\/\//i.test(candidate.url))) {
      throw new Error(`${label}.url must be an HTTP(S) URL or null`);
    }
    ...
    if (!Array.isArray(candidate.warnings) || candidate.warnings.some((warning) => typeof warning !== "string")) {
      throw new Error(`${label}.warnings must be an array of strings`);
    }
...
function expectedKey(entry) {
  return `${compareValue(entry.title)}\u0000${compareValue(entry.organization)}`;
}

export function benchmarkCvImport(reference, providerResult) {
  if (!Array.isArray(reference)) throw new Error("reference must be an array");
  const normalized = normalizeCvImportResult(providerResult);
  const candidatesByKey = new Map();
  for (const candidate of normalized.candidates) {
    const key = expectedKey(candidate);
  ...
  for (const expected of reference) {
    const candidates = candidatesByKey.get(expectedKey(expected)) || [];
    const candidate = candidates.find((item) => !used.has(item.candidateId));
    if (!candidate) {
    ...
    const fields = {
      category: candidate.categoryType === expected.categoryType,
      year: compareValue(candidate.yearLabel) === compareValue(expected.yearLabel),
      title: compareValue(candidate.title) === compareValue(expected.title)
    };
    matches.push({ expected, candidate, fields, needsCorrection: !fields.category || !fields.year || !fields.title || candidate.needsReview });
  }
  const unmatched = normalized.candidates.filter((candidate) => !used.has(candidate.candidateId));
  const exact = (field) => matches.filter((match) => match.fields[field]).length;
  return {
    expected: reference.length,
    found: normalized.candidates.length,
    matched: matches.length,
    missed,
    unmatched,
    matches,
    exactCategory: exact("category"),
    exactYear: exact("year"),
    exactTitle: exact("title"),
    needsCorrection: matches.filter((match) => match.needsCorrection).length
  };
}
...
function formatReport(report) {
  return [
    ...
    "",
    `CATEGORY        ${report.exactCategory} / ${report.matched}`,
    `YEAR            ${report.exactYear} / ${report.matched}`,
    `TITLE           ${report.exactTitle} / ${report.matched}`,
    "",
    `NEEDS REVIEW     ${report.needsCorrection}`
  ].join("\n");
}
...
async function main() {
  ...
  const [text, candidates, reference] = await Promise.all([
    fs.readFile(path.join(root, "sample-cv.txt"), "utf8"),
    readJson(path.join(root, "sample-candidates.json")),
    readJson(path.join(root, "sample-reference.json"))
  ]);
  const result = normalizeCvImportResult(candidates);
  const report = benchmarkCvImport(reference, result);
  console.log(`SOURCE TEXT      ${text.length} characters`);
  console.log(formatReport(report));
}

if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  main().catch((error) => { console.error(`CV IMPORT PROTOTYPE ERROR: ${error.message}`); process.exitCode = 1; });
}
