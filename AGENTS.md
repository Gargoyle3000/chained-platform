...
- For any relevant feature or product change affecting page functionality, editable data, source-of-truth or relationships, visibility/publication, navigation, or linking between CHAINED areas, inspect the affected context in `data/contextual-help.mjs`; update the contextual `[ ? ]` help to match the new behavior, or explicitly report that no help change is needed and why. Purely visual changes do not require help-copy changes unless they alter user understanding or interaction.
...
- Temporary Windows Desktop exception: a current sandbox regression can deny normal Git writes with `.git/index.lock: Permission denied`. Codex continues to inspect, edit, test, and report; use PowerShell for `git add`, commit, and push until the upstream issue changes. Do not change Git permissions or configuration as a workaround, and do not record this tooling limitation in `docs/BUGS.md`.
