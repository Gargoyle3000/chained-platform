...
// Header-only dimension readers: no raster decode or image transformation. They
// deliberately return null when a supported original cannot be proved here;
// Phase 3 then records derivative preparation failure without invalidating the
// already-verified authoritative original.
export function trustedImageDimensions(bytes: Uint8Array, mimeType: string): { width: number; height: number } | null {
  const be16 = (i: number) => (bytes[i]! << 8) | bytes[i + 1]!;
  const be32 = (i: number) => ((bytes[i]! * 0x1000000) + ((bytes[i + 1]! << 16) | (bytes[i + 2]! << 8) | bytes[i + 3]!)) >>> 0;
  ...
  if (mimeType === "image/jpeg") {
    for (let i = 2; i + 9 < bytes.length;) {
      ...
      const length = be16(i + 2); if (length < 2 || i + 2 + length > bytes.length) return null;
      if ((marker >= 0xc0 && marker <= 0xc3) || (marker >= 0xc5 && marker <= 0xc7) || (marker >= 0xc9 && marker <= 0xcb) || (marker >= 0xcd && marker <= 0xcf)) return { width: be16(i + 7), height: be16(i + 5) };
      i += 2 + length;
    }
  }
  if (mimeType === "image/webp" && bytes.length >= 30 && String.fromCharCode(...bytes.slice(0, 4)) === "RIFF") {
    ...
    if (tag === "VP8 " && bytes.length >= 30 && bytes[23] === 0x9d && bytes[24] === 0x01 && bytes[25] === 0x2a) return { width: (bytes[26]! | (bytes[27]! << 8)) & 0x3fff, height: (bytes[28]! | (bytes[29]! << 8)) & 0x3fff };
