...
| `profile-dynamic.js` | Renders published profile data through the dedicated public profile repository | A public query returning published Works only, ordered by `year_sort`, update time, and stable ID while displaying `year_label` |
...
Concretely, `work-store.js` becomes the provider façade or is replaced by a compatible provider module. The Dashboard scripts and `dashboard-work-edit.html` can remain mostly unchanged apart from authentication/loading/error states and the new upload calls. `profile-dynamic.js` and `artwork-dynamic.js` use the dedicated published methods. `agenda.js`, `following.js`, and the Archive scripts move to their own Supabase providers in later phases; their HTML and CSS need not be redesigned.
...
Prototype mode retains the existing IndexedDB Work compatibility but no static public profile or Discover/Following data routes. Local-Supabase mode does not mix browser-local records into public results. Presentations, Events/Agenda, Archive, CV, and Press remain later Supabase integration phases.
